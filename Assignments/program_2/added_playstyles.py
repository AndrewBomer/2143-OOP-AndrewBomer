...
class Player(object):
    def __init__(self,name,num_dice=1,strategy=('Random',7)):
        ...
        self.WinnningScore = 100 # Score to win game
        self.NumDice = num_dice
    ...
    """
    def Roll(self):
        if self.TotalScore / self.WinnningScore >= .8:
    ...
    def RandomRoll(self):
        ...
        for i in range(random.randint(1,self.Strategies['Random'])):
            ...
            if self.TotalScore >= self.WinnningScore:
            	print('%s has reached %d points and is stopping!' % (self.Name, self.WinnningScore))
            	break
    ...
    def SprintToFinish(self):
        ...
        while self.TotalScore < self.WinnningScore:
        	NumRolls += 1
        	roll = self.pig.Roll()
        	if roll == 0:
        		break
